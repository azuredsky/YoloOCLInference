...
__kernel void resetarray(__global float *output, __global float *biases, int size) {
...
__kernel void normalizearray(int N, __global float *x, __global float *mean, __global float *variance, int batch, int filters, int spatial) {
...
__kernel void scalebias(__global float *output, __global float *biases, int n, int size) { //, int arrOffset) {
...
__kernel void addbias(__global float *output, __global float *biases, int n, int size) {

	int filter = get_group_id(1);
	int stepSize = get_local_size(0);
	int offset = get_global_id(0) * stepSize;

	__local float biasesFilt;
	biasesFilt = biases[filter];

	float8 localOutput = vload8(0, &output[filter * size + offset]);
	localOutput += biasesFilt;
	vstore8(localOutput, 0, &output[filter * size + offset]);
}

__kernel void scaleaddbias(__global float *output, __global float *scales, __global float *biases, int n, int size) {
...
__kernel void normscaleaddbias(__global float *output, __global float *scales, __global float *biases, 
	__global float *mean, __global float *variance, int filters, int spatial) {
...
__kernel void leakyactivatearray(__global float *x, __global float *swapBuffer, int n, ACTIVATION a){
	...
	xLocal *= 0;
	vstore8(xLocal, 0, &swapBuffer[index]);
}

__kernel void linearactivatearray(__global float *x, __global float *swapBuffer, int n, ACTIVATION a) {
